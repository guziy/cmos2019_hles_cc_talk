...
\setbeamertemplate{caption}{\vspace{-0.5cm}\insertcaption}
...
\begin{document}
    ...
    \begin{columns}
      \column{0.5\textwidth}
        \begin{figure}
          ...
          \caption{Monthly distribution of area-average HLES for the 1980-2010 period}
        \end{figure}
  ...
  \begin{frame}{Validation: HLES timing}
    ...
    \begin{columns}
      \column{0.5\textwidth}
        \begin{figure}
          \includegraphics[height=0.5\textheight]{hles_histo_all_m9_10_11_12_1_2_3_4_5_percentages.png}
          \caption{Monthly distribution of area-average HLES for the 1980-2010 period}
        \end{figure}

      \column{0.5\textwidth}
        \begin{itemize}
          \item Good agreement between model and observations
          \item Some underestimation during Dec and Jan months
        \end{itemize}
    \end{columns}
  \end{frame}
  ...
  \begin{frame}{Projected changes to the fields impacting HLES}
    \begin{tikzpicture}[indicator/.style={scale=0.8, circle,draw=gray!60, minimum size=3.5em}]
        \node (plot) {\includegraphics[height=0.7\textheight]{projected_changes_to_surf_and_nearsurf_fields_1.png}};
        \node[indicator, below right = 1.5em and 0.1em of plot.north east] (icesymbol) {\color{red}{$\pmb+$}\color{black}{/}\color{red}{$\pmb+$}};
        \node[indicator, below = 1.em of icesymbol] (t2msymbol) {\color{blue}{$\pmb-$}\color{black}{/}\color{red}{$\pmb+$}};
        \node[indicator, below = 1.em of t2msymbol] (prsymbol) {\color{red}{$\pmb+$}};
        \node[indicator, below = 1.em of prsymbol] (snsymbol) {\color{blue}{$\pmb-$}};
        % \node[indicator, below = 0.8em of snsymbol] (caosymbol) {\color{blue}{$\pmb-$}};
        %\node[below=0of plot.south]{};

        \node[above right= -0.5em and 3.2em of plot.north west] (nd) {\footnotesize ND};
        \node[right= 6.4em of nd] (jf) {\footnotesize JF};
        \node[right= 6.4em of jf] (ma) {\footnotesize MA};
    \end{tikzpicture}
    {\footnotesize {\color{red}{$+$}}/{\color{blue}{$-$}} indicate impact of changes to a field on changes to HLES. Total precipitation and snowfall are in mm/day.}
  \end{frame}
  ...
  \begin{frame}{Projected changes to HLES: magnitudes and frequencies}
    ...
    \begin{figure}
      \begin{tikzpicture}
          \node(plot){\includegraphics[width=\textwidth]{projected_changes_to_hles_1.png}};
          \node[above right= -0.5em and 5em of plot.north west] (nd) {\footnotesize ND};
          \node[right= 6.4em of nd] (jf) {\footnotesize JF};
          \node[right= 6.4em of jf] (ma) {\footnotesize MA};
      \end{tikzpicture}
  ...
  \begin{frame}{Mechanisms: 2m air temperature and HLES}
    \begin{figure}
      \begin{tikzpicture}
        ...
        \node[above right= 0.01em and 3.2em of plot.north west] (nd) {\footnotesize ND};
        \node[right= 6.4em of nd] (jf) {\footnotesize JF};
        \node[right= 6.4em of jf] (ma) {\footnotesize MA};
      \end{tikzpicture}
      \caption{Time correlation: 2m air temperature and HLES. 0-degree line is shown in black}
    \end{figure}
